...
QUnit.module('game functionalities', () => {
    QUnit.test('get all games', async(assert) => {
        ...
        for(let i = 0; i < json.lenght - 1; i++) {

            // _ownerId
            assert.ok(json.hasOwnProperty('_ownerId'), 'Property _ownerId exists.');
            assert.strictEqual(json['_ownerId'], 'string', 'Property _ownerId is string.');
            assert.true(json['_userId'].length > 0);
